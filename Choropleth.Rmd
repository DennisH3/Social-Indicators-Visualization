...
# If Population = NA, assign 0
CMA@data$Population[is.na(CMA@data$Population)] <- 0
...
pal <- colorBin("Purples", domain = CMA$Population, bins = bins, na.color = NA)
...
hoverText <- sprintf(
  ...
  CMA@data$`Visible minority`, CMA@data$CMANAME, if (CMA@data$Population == 0){print(NA)}, CMA@data$Age,
  CMA@data$Sex, CMA@data$`Total income groups`, CMA@data$`Generation Status`
) %>% lapply(htmltools::HTML)

# Create the choropleth
leaflet(CMA) %>%
  setView(-106.3, 56.1, zoom = 4.5) %>%
  addTiles() %>%
  addPolygons(
